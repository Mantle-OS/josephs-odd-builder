...
class QMLSD_EXPORT QSdSampleParams : public QSdBaseParam
{
    ...
    QP_RW(float,                        eta,                job::core::safeInfinity()) // // qInf()                      ) // Noise multiplier for ancestral samplers (usually 1.0 or Inf).
    QP_RW(int,                          shiftedTimestep,    0                           ) // Timestep shift (specific to certain experimental schedules).
    QP_RW(float,                        flowShift,          job::core::safeInfinity()) // qInf()                      ) // Flow matching shift value (critical for Flux/SD3 scaling).
