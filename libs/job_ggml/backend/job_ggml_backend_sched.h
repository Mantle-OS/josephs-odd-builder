...
public:
    ...
    {
        return std::make_shared<JobGgmlBackendSched>(
            std::move(backends),
    ...
    {
        return std::make_unique<JobGgmlBackendSched>(
            std::move(backends),
            std::move(bufferTypes),
            graphSize,
            parallel,
...
private:
    ...
    std::vector<JobGgmlBackend::Ptr>           m_backends;
