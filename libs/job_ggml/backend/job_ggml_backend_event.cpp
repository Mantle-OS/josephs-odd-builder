...
{
    if (!device)
        throw std::invalid_argument{ "JobGgmlBackendEvent requires a valid ggml_backend_dev_t" };

    if (!m_event)
        throw std::runtime_error{ "Failed to create GGML backend event" };
}
...
{
    if (!m_event)
        throw std::invalid_argument{ "JobGgmlBackendEvent requires a valid ggml_backend_event_t" };
}
...
{
    if (!m_event)
        throw std::invalid_argument{ "JobGgmlBackendEvent requires a valid ggml_backend_event_ptr" };
}
...
{
    if (!m_event)
        throw std::runtime_error{ "Cannot synchronize an invalid GGML backend event" };
