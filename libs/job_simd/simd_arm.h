...
struct NEON_F {
    ...
    // Set Zero
    [[nodiscard]] static inline f32 zero() { return vdupq_n_f32(0.0f); }

    // Broadcast Scalar (renamed from push -> set1)
    [[nodiscard]] static inline f32 set1(float v) { return vdupq_n_f32(v); }
    static inline i32 set1_i32(int v) { return vdupq_n_s32(v); }


    // Load Unaligned (NEON handles unaligned loads well on ARMv8)
    [[nodiscard]] static inline f32 pull(const float *p) { return vld1q_f32(p); }

    // Load Aligned (Same intrinsic usually, but good for intent)
    [[nodiscard]] static inline f32 pull_a(const float *p) { return vld1q_f32(p); }

    // Store
    static inline void mov(float *p, f32 reg) { vst1q_f32(p, reg); }
    static inline void mov_a(float *p, f32 reg) { vst1q_f32(p, reg); }
    [[nodiscard]] static inline f32 add(f32 a, f32 b) { return vaddq_f32(a, b); }

    // Integer Add
    static inline i32 add_i32(i32 a, i32 b) { return vaddq_s32(a, b); }

    [[nodiscard]] static inline f32 sub(f32 a, f32 b) { return vsubq_f32(a, b); }
    [[nodiscard]] static inline f32 mul(f32 a, f32 b) { return vmulq_f32(a, b); }
    [[nodiscard]] static inline f32 div(f32 a, f32 b) { return vdivq_f32(a, b); }

    // FMA: (a * b) + c
    // Note: vfmaq_f32(c, a, b) performs c + (a * b)
    [[nodiscard]] static inline f32 mul_plus(f32 a, f32 b, f32 c) { return vfmaq_f32(c, a, b); }

    // --- Math Functions ---

    [[nodiscard]] static inline f32 max(f32 a, f32 b) { return vmaxq_f32(a, b); }
    [[nodiscard]] static inline f32 min(f32 a, f32 b) { return vminq_f32(a, b); }
    [[nodiscard]] static inline f32 sqrt(f32 a) { return vsqrtq_f32(a); }
    [[nodiscard]] static inline f32 rsqrt(f32 a) { return vrsqrteq_f32(a); }

    [[nodiscard]] static inline f32 and_ps(f32 a, f32 b) { return vreinterpretq_f32_u32(vandq_u32(vreinterpretq_u32_f32(a), vreinterpretq_u32_f32(b))); }
    [[nodiscard]] static inline f32 or_ps(f32 a, f32 b) { return vreinterpretq_f32_u32(vorrq_u32(vreinterpretq_u32_f32(a), vreinterpretq_u32_f32(b))); }
    [[nodiscard]] static inline f32 xor_ps(f32 a, f32 b) { return vreinterpretq_f32_u32(veorq_u32(vreinterpretq_u32_f32(a), vreinterpretq_u32_f32(b))); }
    static inline f32 gt_ps(f32 a, f32 b) { return vreinterpretq_f32_u32(vcgtq_f32(a, b)); }
    ...
    static inline f32 blendv(f32 a, f32 b, f32 mask) { return vbslq_f32(vreinterpretq_u32_f32(mask), b, a); }

    // Quiet NaN constant
    static inline f32 qnan() { return vreinterpretq_f32_u32(vdupq_n_u32(0x7FC00000)); }
    ...
    [[nodiscard]] static inline f32 floor(f32 a) { return round<RoundingMode::Down>(a); }
    [[nodiscard]] static inline f32 ceil(f32 a) { return round<RoundingMode::Up>(a); }
    [[nodiscard]] static inline f32 trunc(f32 a) { return round<RoundingMode::Truncate>(a); }
    [[nodiscard]] static inline f32 round_nearest(f32 a) { return round<RoundingMode::Nearest>(a); }

    // Unpacking (Interleaving)
    // Corresponds to _mm_unpacklo_ps: [a0, b0, a1, b1]
    [[nodiscard]] static inline f32 unpack_lo(f32 a, f32 b) { return vzip1q_f32(a, b); }
    // Corresponds to _mm_unpackhi_ps: [a2, b2, a3, b3]
    [[nodiscard]] static inline f32 unpack_hi(f32 a, f32 b) { return vzip2q_f32(a, b); }
    template<int Mask>
    ...
    template<int Mask>
    [[nodiscard]] static inline f32 permute_lanes(f32 a, f32 /*b*/) { return a; // Should not be called in 4-wide mode }
    static inline i32 cast_to_int(f32 reg) { return vreinterpretq_s32_f32(reg); }
    static inline f32 cast_to_float(i32 reg) { return vreinterpretq_f32_s32(reg); }
    ...
    // Integer subtract
    static inline i32 sub_i32(i32 a, i32 b) { return vsubq_s32(a, b); }

    // AVX's eq() is actually bitwise AND on floats (VANDPS)
    static inline f32 eq(f32 a, f32 b) { return vandq_f32(a, b); }
