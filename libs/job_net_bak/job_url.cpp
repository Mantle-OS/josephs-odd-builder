...
bool JobUrl::parse(const std::string &urlString)
{
    ...
    try {
        ...
        if (match[3].matched){
            ...
            setPassword(len > 0 ? &*match[3].first : nullptr, len);
        }
        if (match[4].matched)
            m_host = match[4];
        if (match[5].matched)
...
void JobUrl::setbase64EncodePwd(bool b64EncPwd)
{
    m_base64EncodePwd = b64EncPwd;
}

bool JobUrl::base64EncodePwd() const
{
    return m_base64EncodePwd;
}

void JobUrl::setPasswdMode(PasswdMode passwdMode)
{
    m_passwdMode = passwdMode;
}

JobUrl::PasswdMode JobUrl::passwdMode() const
{
    return m_passwdMode;
}
