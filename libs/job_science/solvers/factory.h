...
struct IntegratorStats {
    ...
    real_t        lastStepSeconds{0.0};  ///< Wall-clock, for profiling/GUI
};
...
public:
    using Ptr = std::shared_ptr<IIntegratorStrategy>;
    ...
    virtual void step(IntegratorContext &ctx, real_t dt_s) = 0;
...
[[nodiscard]] IIntegratorStrategy::Ptr makeIntegratorStrategy(const EngineConfig &config, ThreadPool &pool);
