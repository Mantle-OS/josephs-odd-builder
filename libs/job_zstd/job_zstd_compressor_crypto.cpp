...
const job::crypto::JobSecureMem &JobZstdCompressorCrypto::encryptionKey() const noexcept
{
    return m_encryptionKey;
}

void JobZstdCompressorCrypto::setEncryptionKey(const job::crypto::JobSecureMem &key)
{
    m_encryptionKey = key;
}

bool JobZstdCompressorCrypto::hasKeys() const noexcept
{
    return m_encryptionKey.size() == crypto_secretbox_KEYBYTES;
}

bool JobZstdCompressorCrypto::execute()
{
    if (!hasKeys()) {
        setErrorString("Cryptographic pipeline missing a valid encryption key -- call setEncryptionKey()/setEncryptionKeyB64() first.");
        return false;
    }
...
bool JobZstdCompressorCrypto::compressFolder()
{
    if (!hasKeys()) {
        setErrorString("Cryptographic pipeline missing a valid encryption key.");
        return false;
    }
    ...
    job::zstd::JobZstdEncryptingTransport encTransport(dst.rdbuf(), encryptionKey());

    job::zstd::JobZstdIO zstd(&encTransport);
    ...
    // The final, possibly-partial chunk only actually reaches disk here JobZstdIO::close() flushes the zstd frame INTO the transport,
    // but the transport's own tail chunk still needs its own explicit flush.
    if (!encTransport.finish()) {
        setErrorString(encTransport.errorString());
        return false;
    }
...
bool JobZstdCompressorCrypto::compressFile()
{
    if (!hasKeys()) {
        setErrorString("Cryptographic pipeline missing a valid encryption key.");
        return false;
