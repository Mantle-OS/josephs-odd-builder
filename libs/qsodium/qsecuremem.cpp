...
QSecureMem::QSecureMem(size_t size)
{
    ...
    if (!QSodiumInit::isInitialized()) {
        if (QSodiumInit::initialize()) {
            qDebug () << "[QSecureMem] libsodium initialized (lazy init).";
        } else {
            qWarning() << "[QSecureMem] ERROR: libsodium initialization failed — "
    ...
    if (allocate(size)) {
        qDebug() << "[QSecureMem] Allocated " << size << " bytes of secure memory.\n";
    } else {
        qWarning() << "[QSecureMem] Failed to allocate secure memory (" << size << " bytes).\n";
    }
}

QSecureMem::QSecureMem(const QSecureMem &other)
{
    if (other.m_data && other.m_size > 0) {
        if (allocate(other.m_size)) {
            qDebug() << "[QSecureMem] Allocated " << other.m_size << " bytes of secure memory.\n";
        } else {
            qWarning() << "[QSecureMem] Failed to allocate secure memory (" << other.m_size << " bytes).\n";
        }
        copyFrom(other.m_data, other.m_size);
    }
}

QSecureMem &QSecureMem::operator=(const QSecureMem &other)
{
    if (this != &other) {
        if (m_data)
            sodium_free(m_data);
...
QSecureMem::QSecureMem(QSecureMem &&other) noexcept
...
QSecureMem &QSecureMem::operator=(QSecureMem &&other) noexcept
{
    if (this != &other) {
        if (m_data)
            sodium_free(m_data);
...
bool QSecureMem::allocate(size_t size)
{
    if (m_data)
        sodium_free(m_data);
...
void QSecureMem::copyFrom(const void *src, size_t len)
{
    if (len == 0)
        return;

    if (!m_data || len > m_size)
        throw std::runtime_error("QSecureMem: copy exceeds buffer");

    std::memcpy(m_data, src, len);
}

unsigned char *QSecureMem::data() noexcept { return m_data; }
const unsigned char *QSecureMem::data() const noexcept{ return m_data; }

qsizetype QSecureMem::size() const noexcept { return m_size; }

void QSecureMem::clear() noexcept
{
    if (m_data)
        sodium_memzero(m_data, m_size);
}

void QSecureMem::free() noexcept
{
    if (m_data) {
        sodium_memzero(m_data, m_size);
        sodium_free(m_data);
...
#if defined(QSECUREMEM_ALLOW_STRING)
...
QString QSecureMem::toString() const
{
    throw std::runtime_error("QSecureMem::toString() disabled in secure builds");
}

#endif // QSECUREMEM_ALLOW_STRING
...
bool QSecureMem::fromBase64(const QString &encoded, int variant)
{
    ...
    const std::string enc = encoded.toStdString();
    if (enc.empty())
        return false;

    // Max decoded size is roughly (n * 3)/4. Add a small cushion.
    const size_t max_dec = (enc.size() * 3) / 4 + 3;
...
QString QSecureMem::fromBase64toString(const QString &encoded, int variant) const
{
    ...
    size_t outLen = 0;
    std::string enc = encoded.toStdString();
    if (sodium_base642bin(decoded, sizeof(decoded),
                          enc.c_str(), enc.size(),
                          nullptr, &outLen, nullptr,
                          variant) == 0)
    {
        auto ret = std::string(reinterpret_cast<char*>(decoded),
