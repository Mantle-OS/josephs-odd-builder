...
void RopeConfig::setRopeFreqBase(float value)
{
    if (!core::isSafeFinite(value) || value <= 0.0f) {
        throw std::invalid_argument{
            "RopeConfig ropeFreqBase must be finite and greater than zero"
        };
    }
...
void RopeConfig::setRopeFreqScale(float value)
{
    if (!core::isSafeFinite(value) || value <= 0.0f) {
        throw std::invalid_argument{
            "RopeConfig ropeFreqScale must be finite and greater than zero"
        };
    }
...
{
    // ropeDimensionCount (uint32_t) has no invalid representation on its
    // own -- 0 is a legitimate "derive from head dimension" sentinel.
