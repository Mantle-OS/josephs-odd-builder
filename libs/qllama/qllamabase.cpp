...
QLlamaBase::QLlamaBase(QObject *parent):
    QObject(parent)
{

}

void QLlamaBase::debugJson(QJsonDocument::JsonFormat format){
    QJsonDocument jdoc;
    jdoc.setObject(toJson());
    qDebug().noquote() << "[JSON]\n" << jdoc.toJson(format).simplified().data();
}

void QLlamaBase::debugYaml(){
...
bool QLlamaBase::saveToJsonFile(const QString &fileName)
{
    bool ret = false;
    QJsonDocument jdoc;
    jdoc.setObject(toJson());
...
QJsonObject QLlamaBase::loadFromJsonFile(const QString &fileName)
{
    QJsonObject ret;
...
QJsonObject QLlamaBase::toJson() const {
    ...
    for (int i = 0; i < metaObject->propertyCount(); ++i) {
        ...
        if (property.isStored() && property.isReadable()) {
            ...
            if (value.typeId() == QMetaType::Int ||
                ...
                if(!li->isEmpty()){
                    ...
                    for (auto i : li->toVarArray()){
                        if(i.canConvert<QLlamaBase*>()){
                            QLlamaBase *obj = qvariant_cast<QLlamaBase *>(i);
                            if(obj)
                ...
                if(!list.isEmpty()){
                    ...
                    for (const QVariant &item : list) {
                        if (item.canConvert<QLlamaBase *>()) {
                            QLlamaBase *obj = qvariant_cast<QLlamaBase *>(item);
                            if (obj)
                                jsonArray.append(obj->toJson());
                        }else{
                ...
                if(!map.isEmpty()){
                    ...
                    for (auto it = map.begin(); it != map.end(); ++it) {
                        if (it.value().canConvert<QLlamaBase *>()) {
                            QLlamaBase *obj = qvariant_cast<QLlamaBase *>(it.value());
                            if (obj)
            ...
            }else if (value.canConvert<QLlamaBase *>()){
                // Handle complex types
                QLlamaBase *obj = qvariant_cast<QLlamaBase *>(value);
                if (obj) {
...
void QLlamaBase::fromJson(const QJsonObject &jsonObject) {
    const QMetaObject *metaObject = this->metaObject();
    for (int i = 0; i < metaObject->propertyCount(); ++i) {
        QMetaProperty property = metaObject->property(i);
        if (property.isStored() /*&& property.isWritable() && jsonObject.contains(property.name())*/ ) {
            ...
            if (value.typeId() == QMetaType::Int ||
                ...
                if(!jsonArray.isEmpty()){
                    ...
                    for (const QJsonValue &jsonValue : jsonArray) {
                        if (jsonValue.isObject()) {
                            QLlamaBase *item = new QLlamaBase(); // MAKE SURE YOU DELETE THIS IN THE Variant list
                ...
                if(!mapObject.isEmpty()){
                    ...
                    for (auto it = mapObject.begin(); it != mapObject.end(); ++it) {
                        if (it.value().isObject()) {
                            QLlamaBase *item = new QLlamaBase(); // MAKE SURE YOU DELETE THIS IN THE Variant MAP
...
bool QLlamaBase::saveToYamlFile(const QString &fileName)
{
...
YAML::Node QLlamaBase::loadFromYamlFile(const QString &fileName)
{
...
YAML::Node QLlamaBase::toYaml() const {
    YAML::Node yamlNode;
    const QMetaObject *metaObject = this->metaObject();
    for (int i = 0; i < metaObject->propertyCount(); ++i) {
        QMetaProperty property = metaObject->property(i);
...
void QLlamaBase::fromYaml(const YAML::Node &yamlNode)
{
    const QMetaObject *metaObject = this->metaObject();
    for (int i = 0; i < metaObject->propertyCount(); ++i) {
        ...
        if (property.isStored() && yamlNode[property.name()] ){ //&& property.isWritable() && yamlNode[property.name()]) {
            ...
            // Handle QList
                ...
                if(listNode.size() > 0){
                    ...
                    for (const YAML::Node &itemNode : listNode) {
                        if (itemNode.IsMap()) {
                            QLlamaBase *item = new QLlamaBase(); // you OWN this
