    ...
    m_backend(std::make_unique<Backend>())
{
    ...
    m_backend->epollFd = ::epoll_create1(EPOLL_CLOEXEC);

    if (m_backend->epollFd == -1) {
        JOB_LOG_ERROR("[JobIoAsyncThread] Failed to create epoll FD: {}", std::strerror(errno));
        return;
    }

    // m_backend->eventFd = ::eventfd(0, EFD_NONBLOCK);
    m_backend->eventFd = ::eventfd(0, EFD_NONBLOCK | EFD_CLOEXEC);

    if (m_backend->eventFd == -1) {
        JOB_LOG_ERROR("[JobIoAsyncThread] Failed to create eventfd: {}", std::strerror(errno));
    ...
    if (::epoll_ctl(m_backend->epollFd, EPOLL_CTL_ADD, m_backend->eventFd, &ev) == -1) {
        JOB_LOG_ERROR("[JobIoAsyncThread] Failed to add eventfd to epoll: {}", std::strerror(errno));
...
JobIoAsyncThread::Ptr JobIoAsyncThread::createShared()
{
    return std::make_shared<JobIoAsyncThread>();
}

JobIoAsyncThread::UPtr JobIoAsyncThread::createUniq()
{
    return std::make_unique<JobIoAsyncThread>();
}
...
void JobIoAsyncThread::stop()
{
    ...
    if (m_backend->eventFd != -1) {
        uint64_t val = 1;
        const ssize_t s = ::write(m_backend->eventFd, &val, sizeof(val));

        if (s != sizeof(val) && errno != EAGAIN)
            JOB_LOG_ERROR("[JobIoAsyncThread] stop() failed to write to eventfd: {}", std::strerror(errno));
    }
...
void JobIoAsyncThread::post(std::function<void()> task, int priority)
{
    ...
    if (m_backend->eventFd != -1) {
        ...
        if (s != sizeof(val) && errno != EAGAIN)
            JOB_LOG_WARN("[JobIoAsyncThread] Failed to write to eventfd: {}", std::strerror(errno));
    }
}
...
bool JobIoAsyncThread::registerFD(int fd, IOEvent events, IOEventCallback callback)
{
    ...
    if (::epoll_ctl(m_backend->epollFd, EPOLL_CTL_ADD, fd, &ev) == -1) {
        JOB_LOG_ERROR("[JobIoAsyncThread] epoll_ctl ADD failed for fd {}: {}", fd, std::strerror(errno));
        return false;
    }
...
bool JobIoAsyncThread::unregisterFD(int fd)
{
    ...
    if (::epoll_ctl(m_backend->epollFd, EPOLL_CTL_DEL, fd, &ev) == -1) {
        if (errno != ENOENT)
            JOB_LOG_WARN("[JobIoAsyncThread] epoll_ctl DEL failed for fd {}: {}", fd, std::strerror(errno));
    }
...
void JobIoAsyncThread::processIOEvents(int eventCount)
{
    EventCallback callbacksToRun;
    callbacksToRun.reserve(eventCount);

    {
        std::scoped_lock lock(m_ioMutex);

        for (int i = 0; i < eventCount; ++i) {
            const int fd = m_backend->epollEvents[i].data.fd;
            const auto ev = m_backend->epollEvents[i].events;

            if (fd == m_backend->eventFd) {
                for (;;) {
                    uint64_t val;
                    const ssize_t s = ::read(m_backend->eventFd, &val, sizeof(val));
...
void JobIoAsyncThread::loop(std::stop_token token,
                            [[maybe_unused]] std::chrono::milliseconds idleHeartbeat)
{
    if (m_backend->epollFd == -1) {
    ...
    while (!token.stop_requested()) {
        ...
        auto nextWakeup = calculateNextWakeup();
        int timeoutMs = -1;

        if (!m_queue.isEmpty()) {
            timeoutMs = 0;
        } else if (nextWakeup != std::chrono::milliseconds::max()) {
            using Rep = std::chrono::milliseconds::rep;
            timeoutMs = static_cast<int>(std::max(static_cast<Rep>(0), nextWakeup.count()));
        }

        const int eventCount = ::epoll_wait(
            m_backend->epollFd,
            m_backend->epollEvents.data(),
            m_backend->epollEvents.size(),
            timeoutMs
            );
        ...
        if (eventCount < 0) {
            ...
            JOB_LOG_ERROR("[JobIoAsyncThread] epoll_wait error: {}", std::strerror(errno));
            continue;
        }

        if (eventCount == static_cast<int>(m_backend->epollEvents.size())) {
