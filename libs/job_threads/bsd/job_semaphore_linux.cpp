...
static sem_t *inPlaceSem(unsigned char *storage) noexcept
{
...
JobSem::~JobSem()
{
    ...
    if (!m_linked && m_initedInPlace) {
        if (::sem_destroy(inPlaceSem(m_inPlaceStorage)) != 0)
...
JobSemRet JobSem::init(unsigned value, bool pshared)
{
    ...
    int rc = ::sem_init(inPlaceSem(m_inPlaceStorage), pshared ? 1 : 0, value);
    if (rc == 0) {
        m_sem           = inPlaceSem(m_inPlaceStorage);
        m_ready         = true;
        m_linked        = false;
...
JobSemRet JobSem::destroy()
{
    ...
    if (!m_linked && m_initedInPlace) {
        if (::sem_destroy(inPlaceSem(m_inPlaceStorage)) != 0)
            result = fromPlatformError(errno);
    } else {
        if (::sem_close(static_cast<sem_t*>(m_sem)) != 0)
            result = fromPlatformError(errno);
