...
[[nodiscard]] inline core::real_t activate(job::ai::base::ActivationType type, core::real_t x, core::real_t alpha = 0)
{
    ...
    case ActivationType::Maxout:
        JOB_LOG_ERROR("[ActivationMath]: BRO call job::ai::base::maxout(range) on its own"); // I guess we could call max now
        break;
    case ActivationType::GDN:
        JOB_LOG_ERROR("[ActivationMath]: Sure lets pass in things that need vectors into a pointwise function ......."); // I guess we could call this as well now
        break;
    default:
        return x;
    }
