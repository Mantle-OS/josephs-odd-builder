                           ...
                           [[maybe_unused]]const AdapterCtx &ctx)
{
    ...
    std::vector<float, cords::AlignedAllocator<float, 64>> scratch(B * elementsPerBatch);
    ...
    job::threads::parallel_for(pool, size_t{0}, totalSims, [&](size_t idx) {
    ...
    job::threads::parallel_for(pool, size_t{0}, size_t(B), [&](size_t b) {
        ...
        job::ai::comp::transpose(scratchBatch, outBatch, D, S);
    });
}
}
