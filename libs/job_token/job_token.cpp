...
{
    IToken::UPtr token = TokenFactory::create(provider, modelPath);
    if (!token) {
        JOB_LOG_ERROR("Failed to create token provider for '{}'", modelPath.string());
        return false;
    }

    return setToken(std::move(token));
}
...
{
    IToken::UPtr token = TokenFactory::create(provider, modelPath, tokenizerPath);
    if (!token) {
        JOB_LOG_ERROR(
...
bool JobToken::setToken(IToken::UPtr token)
{
    clearRuntime();
    m_token.reset();
...
bool JobToken::configure()
{
    ...
    // Chat runtime
    m_chatEngine = ChatEngine::createUniq();
    if (!m_chatEngine) {
        JOB_LOG_ERROR("Failed to create ChatEngine");
        return false;
    }

    if (!m_token->chatTemplate().empty())
        m_chatEngine->setTemplate(m_token->chatTemplate());
...
std::vector<TokenId> JobToken::encode(std::string_view text) const
{
    ...
    const SpecialTokens *specialTokens = m_token->specialTokens();
    std::size_t position = 0;

    while (position < text.size()) {
        const std::optional<SpecialTokens::Match> match = specialTokens ?
                                                              specialTokens->findNext(text, position) :
                                                              std::nullopt;

        if (!match) {
            encodeNormalText(text.substr(position), output);
            break;
        }

        if (match->position > position)
            encodeNormalText(text.substr(position, match->position - position), output);

        output.push_back(match->id);
        position = match->position + match->length;
    }

    if (m_token->addBosToken() && specialTokens) {
        const TokenId bosId = specialTokens->bosId();
        if (bosId != kInvalidToken)
            output.insert(output.begin(), bosId);
    }

    if (m_token->addEosToken() && specialTokens) {
        const TokenId eosId = specialTokens->eosId();
...
void JobToken::encodeNormalText(std::string_view text, std::vector<TokenId> &output) const
{
    if (text.empty())
        return;

    UnicodeNormalizer::Options options;
    options.addDummyPrefixSpace = m_token->addPrefixSpace();

    const std::string normalized = m_normalizer->normalize(text, options);
    const std::vector<std::string_view> chunks = m_splitter->split(normalized);

    if (chunks.empty()) {
        const ByteSymbols symbols = m_byteEncoder->encode(normalized);
        const std::vector<TokenId> encoded = m_algorithm->encode(symbols);
        output.insert(output.end(), encoded.begin(), encoded.end());
        return;
    }

    for (const std::string_view chunk : chunks) {
        const ByteSymbols symbols = m_byteEncoder->encode(chunk);
        const std::vector<TokenId> encoded = m_algorithm->encode(symbols);
        output.insert(output.end(), encoded.begin(), encoded.end());
    }
}
