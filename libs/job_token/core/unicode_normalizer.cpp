...
bool UnicodeNormalizer::nextCodepoint(std::string_view text, std::size_t &offset, std::uint32_t &outCodepoint) noexcept
{
    ...
    if (b0 >= 0xE0 && b0 <= 0xEF) {
        ...
        outCodepoint = (static_cast<std::uint32_t>(b0 & 0x0F) << 12) |
    ...
    if (b0 >= 0xF0 && b0 <= 0xF4) {
        ...
        outCodepoint = (static_cast<std::uint32_t>(b0 & 0x07) << 18) |
...
std::size_t UnicodeNormalizer::encodeCodepoint(std::uint32_t codepoint, char *outBuffer) noexcept
{
    ...
    auto *bytes = reinterpret_cast<std::uint8_t *>(outBuffer);
    if (codepoint <= 0x7F) {
    ...
    if (codepoint <= 0x7FF) {
        bytes[0] = static_cast<std::uint8_t>(0xC0 | (codepoint >> 6));
        bytes[1] = static_cast<std::uint8_t>(0x80 | (codepoint & 0x3F));
        return 2;
    }

    if (codepoint <= 0xFFFF) {
        bytes[0] = static_cast<std::uint8_t>(0xE0 | (codepoint >> 12));
        bytes[1] = static_cast<std::uint8_t>(0x80 | ((codepoint >> 6) & 0x3F));
        bytes[2] = static_cast<std::uint8_t>(0x80 | (codepoint & 0x3F));
        return 3;
    }

    if (codepoint <= 0x10FFFF) {
        bytes[0] = static_cast<std::uint8_t>(0xF0 | (codepoint >> 18));
        bytes[1] = static_cast<std::uint8_t>(0x80 | ((codepoint >> 12) & 0x3F));
        bytes[2] = static_cast<std::uint8_t>(0x80 | ((codepoint >> 6) & 0x3F));
        bytes[3] = static_cast<std::uint8_t>(0x80 | (codepoint & 0x3F));
        return 4;
    }
...
std::string UnicodeNormalizer::normalize(std::string_view text, const Options &options)
{
    ...
    if (options.replaceSpacesWithMarker)
        return escapeSpaces(normalized, options.addDummyPrefixSpace);

    if (options.addDummyPrefixSpace && !normalized.empty() && normalized.front() != ' ')
