...
public:
    ...
    [[nodiscard]] static bool nextUtf8(std::string_view input, std::size_t &offset, char32_t &codePoint) noexcept
    {
...
private:
    [[nodiscard]] static constexpr bool isDirectByte(std::uint16_t byte) noexcept
    {
    ...
    [[nodiscard]] static EncodeTable createEncodeTable()
    {
        ...
        for (std::uint16_t byte = 0; byte < 256; ++byte) {
            if (isDirectByte(byte)) {
                table[byte] = static_cast<char32_t>(byte);
                continue;
            }

            table[byte] = nextCodePoint;
            ++nextCodePoint;
        }
    ...
    {
        DecodeTable table{};
        table.fill(static_cast<std::int16_t>( -1));
        const EncodeTable &encode = encodeTable();
        for (std::uint16_t byte = 0; byte < 256; ++byte) {
            const char32_t codePoint = encode[byte];
            if (codePoint < static_cast<char32_t>(table.size())) {
                table[codePoint] = static_cast<std::int16_t>(byte);
            }
        }
...
ByteSymbols Gpt2ByteEncoder::encode(std::string_view input) const
{
    const Gpt2ByteEncoderTable::EncodeTable &table = Gpt2ByteEncoderTable::encodeTable();
    ByteSymbols output;
    // One tokenizer symbol is produced for every input byte.
    output.reserve(input.size());
    for (const char character : input) {
        const auto byte = static_cast<std::uint8_t>(character);
        output.emplace_back(Gpt2ByteEncoderTable::utf8(table[byte]));
    }

    return output;
}

std::string Gpt2ByteEncoder::decode(std::span<const std::string> input) const
{
    const auto &table = Gpt2ByteEncoderTable::decodeTable();
    ...
    for (const std::string &symbol : input) {
        ...
        while (offset < symbol.size()) {
            ...
            if (!Gpt2ByteEncoderTable::nextUtf8(symbol, offset, codePoint)) {
                return {};
            }
            ...
            const std::int16_t byte = table[codePoint];
            if (byte < 0)
                return {};

            output.push_back(static_cast<char>( static_cast<std::uint8_t>(byte)));
        }
    }
