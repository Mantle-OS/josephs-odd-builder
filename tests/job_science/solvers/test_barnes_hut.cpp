...
#include <ctx/job_sporadic_ctx.h>
...
using Integrator = job::science::BarnesHutForceCalculator<Particle, Vec3f, float>;
...
TEST_CASE("Barnes-Hut matches direct force for 2-body with tiny theta", "[barnes_hut][gravity]")
{
    ...
    Integrator bh(sched.pool, get_pos, get_mass, theta, G, eps2);
...
TEST_CASE("Barnes-Hut: empty and single-particle are safe", "[barnes_hut][edge]")
{
    ...
    Integrator bh(sched.pool, get_pos, get_mass, 0.5f, 1.0f, 1e-4f);
...
TEST_CASE("Barnes-Hut handles many particles at identical positions (no kablamo)", "[barnes_hut][degenerate]")
{
    ...
    for (std::size_t i = 0; i < ps.size(); ++i) {
        ...
        REQUIRE(job::core::isSafeFinite(F_bh[i].x));
        REQUIRE(job::core::isSafeFinite(F_bh[i].y));
        REQUIRE(job::core::isSafeFinite(F_bh[i].z));
