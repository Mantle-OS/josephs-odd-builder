...
using Integrator = job::science::JobRK4Integrator<Particle, Vec3f, float>;
...
TEST_CASE("RK4 Integrator: Edge Cases", "[threading][science][rk4][edge]")
{
...

    {
        ...
        Integrator integ(sched.pool, &empty_particles,
                         nullptr,
                         nullptr,
                         nullptr,
                         pop_goes_the_wessel
...
TEST_CASE("RK4 Integrator: handles many particles", "[threading][science][rk4][stress]")
{
    ...
    for (const auto &p : ps) {
        ...
        REQUIRE(job::core::isSafeFinite(p.position.z));
    }
