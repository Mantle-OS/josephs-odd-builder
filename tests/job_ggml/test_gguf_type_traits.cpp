...
enum class UnsupportedEnum : std::uint32_t {
    Value = 1
};
...
{
    REQUIRE(
        JobGgufTypeTraits::typeFor<std::uint8_t>() ==
        JobGgufType::UInt8
        );
...
TEST_CASE(
    "GGUF type traits map signed integer types",
    "[gguf][type_traits][usage][signed]"
    )
{
    REQUIRE(
        JobGgufTypeTraits::typeFor<std::int8_t>() ==
...
TEST_CASE(
    "GGUF type traits map floating point types",
    "[gguf][type_traits][usage][floating]"
    )
{
    REQUIRE(
        JobGgufTypeTraits::typeFor<float>() ==
...
TEST_CASE(
    "GGUF type traits map boolean and string types",
    "[gguf][type_traits][usage][special]"
    )
{
    REQUIRE(
        JobGgufTypeTraits::typeFor<bool>() ==
...
TEST_CASE(
    "GGUF type traits remove cv and reference qualifiers",
    "[gguf][type_traits][usage][qualifiers]"
    )
{
    REQUIRE(
        JobGgufTypeTraits::typeFor<const std::uint32_t>() ==
        JobGgufType::UInt32
        );

    REQUIRE(
        JobGgufTypeTraits::typeFor<volatile std::uint32_t>() ==
        JobGgufType::UInt32
        );

    REQUIRE(
        JobGgufTypeTraits::typeFor<const volatile std::uint32_t>() ==
        JobGgufType::UInt32
        );
    ...
    REQUIRE(
        JobGgufTypeTraits::typeFor<const std::string &>() ==
        JobGgufType::String
        );
}

TEST_CASE(
    "GGUF type traits map directly to native GGUF types",
    "[gguf][type_traits][usage][native]"
    )
{
    REQUIRE(
        toGgufType(
...
TEST_CASE(
    "GGUF type traits agree with fixed width type sizes",
    "[gguf][type_traits][usage][size]"
    )
{
    REQUIRE(
        ggufTypeSize(
...
TEST_CASE(
    "GGUF type traits agree with type names",
    "[gguf][type_traits][usage][name]"
    )
{
    REQUIRE(
        ggufTypeName(
...
TEST_CASE(
    "GGUF value concept accepts exactly the supported primitive types",
    "[gguf][type_traits][edge][concept]"
    )
{
    STATIC_REQUIRE(
        JobGgufValueType<std::uint8_t>
        );

    STATIC_REQUIRE(
        JobGgufValueType<std::int8_t>
        );

    STATIC_REQUIRE(
        JobGgufValueType<std::uint16_t>
        );

    STATIC_REQUIRE(
        JobGgufValueType<std::int16_t>
        );

    STATIC_REQUIRE(
        JobGgufValueType<std::uint32_t>
        );

    STATIC_REQUIRE(
        JobGgufValueType<std::int32_t>
        );

    STATIC_REQUIRE(
        JobGgufValueType<std::uint64_t>
        );

    STATIC_REQUIRE(
        JobGgufValueType<std::int64_t>
        );

    STATIC_REQUIRE(
        JobGgufValueType<float>
        );

    STATIC_REQUIRE(
        JobGgufValueType<double>
        );

    STATIC_REQUIRE(
        JobGgufValueType<bool>
        );

    STATIC_REQUIRE(
        JobGgufValueType<std::string>
        );
}

TEST_CASE(
    "GGUF value concept rejects unsupported primitive and object types",
    "[gguf][type_traits][edge][concept][invalid]"
    )
{
    STATIC_REQUIRE_FALSE(
        JobGgufValueType<char>
        );

    STATIC_REQUIRE_FALSE(
        JobGgufValueType<wchar_t>
        );

    STATIC_REQUIRE_FALSE(
        JobGgufValueType<char16_t>
        );

    STATIC_REQUIRE_FALSE(
        JobGgufValueType<char32_t>
        );

    STATIC_REQUIRE_FALSE(
        JobGgufValueType<long double>
        );

    STATIC_REQUIRE_FALSE(
        JobGgufValueType<std::byte>
        );

    STATIC_REQUIRE_FALSE(
        JobGgufValueType<void *>
        );

    STATIC_REQUIRE_FALSE(
        JobGgufValueType<UnsupportedValue>
        );

    STATIC_REQUIRE_FALSE(
        JobGgufValueType<UnsupportedEnum>
        );
}

TEST_CASE(
    "GGUF value concept rejects containers as scalar element types",
    "[gguf][type_traits][edge][concept][container]"
    )
{
    STATIC_REQUIRE_FALSE(
        JobGgufValueType<
