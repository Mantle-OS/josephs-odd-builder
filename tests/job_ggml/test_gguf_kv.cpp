...
TEST_CASE("GGUF key value factories create owned scalar values", "[gguf][kv][usage][factory]")
{
    auto shared = JobGgufKv::createShared("job.shared", std::uint32_t{42});
    auto unique = JobGgufKv::createUniq("job.unique", std::string{"glasses"});
    ...
    REQUIRE(shared->value<std::uint32_t>() == std::uint32_t{42});

    REQUIRE(unique->value<std::string>() == "glasses");
}

TEST_CASE("GGUF key value stores every supported unsigned scalar type", "[gguf][kv][usage][scalar][unsigned]")
{
    SECTION("uint8")
    {
        JobGgufKv value{ "job.u8", std::uint8_t{8} };
        REQUIRE(value.isValid());
        REQUIRE(value.key() == "job.u8");
        REQUIRE(value.isScalar());
        REQUIRE_FALSE(value.isArray());
        REQUIRE(value.serializedType() == JobGgufType::UInt8);
        REQUIRE(value.type() == JobGgufType::UInt8);
        REQUIRE(value.elementCount() == 1);
        REQUIRE(value.value<std::uint8_t>() == std::uint8_t{8} );
    }

    SECTION("uint16")
    {
        JobGgufKv value{ "job.u16", std::uint16_t{1600} };
        REQUIRE(value.isValid());
        REQUIRE(value.isScalar());
        REQUIRE(value.type() == JobGgufType::UInt16);
        REQUIRE(value.value<std::uint16_t>() == std::uint16_t{1600} );
    }

    SECTION("uint32")
    {
        JobGgufKv value{ "job.u32", std::uint32_t{320000} };
        REQUIRE(value.isValid());
        REQUIRE(value.isScalar());
        REQUIRE(value.type() == JobGgufType::UInt32);
        REQUIRE(value.value<std::uint32_t>() == std::uint32_t{320000} );
    }

    SECTION("uint64")
    {
        JobGgufKv value{ "job.u64", std::uint64_t{64000000} };
        REQUIRE(value.isValid());
        REQUIRE(value.isScalar());
        REQUIRE(value.type() == JobGgufType::UInt64);
        REQUIRE(value.value<std::uint64_t>() == std::uint64_t{64000000});
    }
}

TEST_CASE( "GGUF key value stores every supported signed scalar type", "[gguf][kv][usage][scalar][signed]")
{
    SECTION("int8")
    {
        JobGgufKv value{ "job.i8", std::int8_t{-8} };
        REQUIRE(value.isValid());
        REQUIRE(value.isScalar());
        REQUIRE(value.type() == JobGgufType::Int8);
        REQUIRE(value.value<std::int8_t>() == std::int8_t{-8} );
    }

    SECTION("int16")
    {
        JobGgufKv value{ "job.i16", std::int16_t{-1600} };
        REQUIRE(value.isValid());
        REQUIRE(value.isScalar());
        REQUIRE(value.type() == JobGgufType::Int16);
        REQUIRE(value.value<std::int16_t>() == std::int16_t{-1600});
    }

    SECTION("int32")
    {
        JobGgufKv value{ "job.i32", std::int32_t{-320000} };
        REQUIRE(value.isValid());
        REQUIRE(value.isScalar());
        REQUIRE(value.type() == JobGgufType::Int32);
        REQUIRE(value.value<std::int32_t>() == std::int32_t{-320000});
    }

    SECTION("int64")
    {
        JobGgufKv value{ "job.i64", std::int64_t{-64000000} };
        REQUIRE(value.isValid());
        REQUIRE(value.isScalar());
        REQUIRE(value.type() == JobGgufType::Int64);
        REQUIRE(value.value<std::int64_t>() == std::int64_t{-64000000});
    }
}

TEST_CASE("GGUF key value stores floating point scalar types", "[gguf][kv][usage][scalar][floating]")
{
    SECTION("float32")
    {
        JobGgufKv value{ "job.f32", 3.25f };
        REQUIRE(value.isValid());
        REQUIRE(value.isScalar());  REQUIRE( value.type() == JobGgufType::Float32);
        REQUIRE(value.value<float>() == 3.25f);
    }

    SECTION("float64")
    {
        JobGgufKv value{ "job.f64", 6.5 };
        REQUIRE(value.isValid());
        REQUIRE(value.isScalar());
...
TEST_CASE("GGUF key value stores boolean scalar metadata", "[gguf][kv][usage][scalar][bool]")
{
    JobGgufKv enabled{ "job.enabled", true };
    JobGgufKv disabled{ "job.disabled", false };
    ...
    REQUIRE(enabled.serializedType() == JobGgufType::Bool);
    REQUIRE(enabled.type() == JobGgufType::Bool);

    REQUIRE(enabled.value<bool>());
    REQUIRE_FALSE(disabled.value<bool>());
}

TEST_CASE("GGUF key value stores string scalar metadata", "[gguf][kv][usage][scalar][string]")
{
    JobGgufKv value{ "general.name", std::string{ "Joseph's Odd Builder" } };

    REQUIRE(value.isValid());
    REQUIRE(value.key() == "general.name");
    REQUIRE(value.isScalar());
    REQUIRE_FALSE(value.isArray());
    ...
    REQUIRE(value.serializedType() == JobGgufType::String);
    REQUIRE(value.type() == JobGgufType::String);
    REQUIRE(value.elementCount() == 1);

    REQUIRE(value.value<std::string>() == "Joseph's Odd Builder");
}

TEST_CASE("GGUF key value stores unsigned integer arrays", "[gguf][kv][usage][array][unsigned]")
{
    SECTION("uint8")
    {
        const std::vector<std::uint8_t> expected{ 1, 2, 3, 4 };
        JobGgufKv value{ "job.u8.array", expected };
        REQUIRE(value.isValid());
        ...
        REQUIRE(value.values<std::uint8_t>() == expected);
    }

    SECTION("uint16")
    {
        const std::vector<std::uint16_t> expected{ 16, 32, 64, 128 };
        JobGgufKv value{ "job.u16.array", expected };
        REQUIRE(value.isValid());
        REQUIRE(value.isArray());
        REQUIRE(value.type() == JobGgufType::UInt16);
        REQUIRE(value.values<std::uint16_t>() == expected);
    }

    SECTION("uint32")
    {
        const std::vector<std::uint32_t> expected{ 64, 128, 256, 512 };
        JobGgufKv value{ "job.u32.array", expected };
        REQUIRE(value.isValid());
        REQUIRE(value.isArray());
        REQUIRE(value.type() == JobGgufType::UInt32);
        REQUIRE(value.values<std::uint32_t>() == expected);
    }

    SECTION("uint64")
    {
        const std::vector<std::uint64_t> expected{ 1024, 2048, 4096, 8192 };
        JobGgufKv value{ "job.u64.array", expected };
        REQUIRE(value.isValid());
        REQUIRE(value.isArray());
        REQUIRE(value.type() == JobGgufType::UInt64);
        REQUIRE(value.values<std::uint64_t>() == expected);
    }
}

TEST_CASE("GGUF key value stores signed integer arrays", "[gguf][kv][usage][array][signed]")
{
    SECTION("int8")
    {
        const std::vector<std::int8_t> expected{ -4, -2, 0, 2, 4 };
        JobGgufKv value{ "job.i8.array", expected };
        REQUIRE(value.isValid());
        REQUIRE(value.isArray());
        REQUIRE(value.type() == JobGgufType::Int8);
        REQUIRE(value.values<std::int8_t>() == expected);
    }

    SECTION("int16")
    {
        const std::vector<std::int16_t> expected{ -128, -64, 64, 128 };
        JobGgufKv value{ "job.i16.array", expected };
        REQUIRE(value.isValid());
        REQUIRE(value.isArray());
        REQUIRE(value.type() == JobGgufType::Int16);
        REQUIRE(value.values<std::int16_t>() == expected);
    }

    SECTION("int32")
    {
        const std::vector<std::int32_t> expected{ -4096, -2048, 2048, 4096 };
        JobGgufKv value{ "job.i32.array", expected };
        REQUIRE(value.isValid());
        REQUIRE(value.isArray());
        REQUIRE(value.type() == JobGgufType::Int32);
        REQUIRE(value.values<std::int32_t>() == expected);
    }

    SECTION("int64")
    {
        const std::vector<std::int64_t> expected{ -64000000, -32000000, 32000000, 64000000 };
        JobGgufKv value{ "job.i64.array", expected };
        REQUIRE(value.isValid());
        REQUIRE(value.isArray());
        REQUIRE(value.type() == JobGgufType::Int64);
        REQUIRE(value.values<std::int64_t>() == expected);
    }
}

TEST_CASE("GGUF key value stores floating point arrays", "[gguf][kv][usage][array][floating]")
{
    SECTION("float32")
    {
        const std::vector<float> expected{ -1.5f, 0.0f, 1.5f, 3.0f };
        JobGgufKv value{ "job.f32.array", expected };
        REQUIRE(value.isValid());
        REQUIRE(value.isArray());
        REQUIRE(value.type() == JobGgufType::Float32);
        REQUIRE(value.values<float>() == expected);
    }

    SECTION("float64")
    {
        const std::vector<double> expected{ -3.25, 0.0, 3.25, 6.5 };
        JobGgufKv value{ "job.f64.array", expected };
        REQUIRE(value.isValid());
        REQUIRE(value.isArray());
        REQUIRE(value.type() == JobGgufType::Float64);
        REQUIRE(value.values<double>() == expected);
    }
}

TEST_CASE("GGUF key value stores boolean arrays", "[gguf][kv][usage][array][bool]")
{
    const std::vector<bool> expected{ true, false, true, true, false };
    JobGgufKv value{ "job.bool.array", expected };
    REQUIRE(value.isValid());
    REQUIRE(value.isArray());
    REQUIRE_FALSE(value.isScalar());
    ...
    REQUIRE(value.elementCount() == expected.size());
    REQUIRE(value.values<bool>() == expected);
}
...
{
    REQUIRE_THROWS_AS((JobGgufKv{ "", std::uint32_t{42}}), std::invalid_argument);
    REQUIRE_THROWS_AS((JobGgufKv{ std::string{}, std::string{"value"}}), std::invalid_argument);
    REQUIRE_THROWS_AS((JobGgufKv{ "", std::vector<std::uint32_t>{ 1, 2, 3 } }), std::invalid_argument);
}
...
TEST_CASE("GGUF scalar key value construction performance", "[gguf][kv][benchmark][construction][scalar]")
{
    BENCHMARK("construct one uint64 GGUF value") {
        return JobGgufKv{ "job.benchmark.scalar", std::uint64_t{42}};
    };
}
