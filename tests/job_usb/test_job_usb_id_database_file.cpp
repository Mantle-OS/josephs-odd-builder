...
//////////////////////////////////////////////////////////
// Block 1: Database load / lookup
//////////////////////////////////////////////////////////

TEST_CASE("JobUsbIdDatabase loads and finalizes usb.ids with JobFile",
          "[job_usb][id][database][file]")
{
    ...
    REQUIRE(database.load(path, false));
    ...
    REQUIRE(database.productName(0x1532, 0x0064) == "Basilisk");
    ...
    BENCHMARK("JobUsbIdDatabase file vendor lookup") {
        return database.vendorName(0x046d);
    };

    BENCHMARK("JobUsbIdDatabase file product lookup") {
        return database.productName(0x046d, 0xc534);
    };

    BENCHMARK("JobUsbIdDatabase file protocol lookup") {
...
#ifdef USE_JOSEPHS_LOCAL_IDS

//////////////////////////////////////////////////////////
// Block 2: Real database
//////////////////////////////////////////////////////////

TEST_CASE("JobUsbIdDatabase loads and benchmarks the whole database with JobFile",
          "[job_usb][id][database][file][local][benchmark]")
{
    auto &database = JobUsbIdDatabase::instance();
    ...
    BENCHMARK("Full DB file: Vendor lookup (cycled targets)") {
    ...
    BENCHMARK("Full DB file: Product lookup (cycled targets)") {
        const auto &[vendor, product] =
            test_products[(pIdx++) % std::size(test_products)];
