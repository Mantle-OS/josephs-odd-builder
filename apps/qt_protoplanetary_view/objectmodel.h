...
QMap<Key, T> qMapFromVariant(const QVariantMap &map) {
    static_assert(std::is_convertible_v<QString, Key>,
                  "qMapFromVariant only supports keys that can be constructed from QString");

    static_assert(std::is_pointer_v<T>,
                  "qMapFromVariant assumes T is a pointer to QObject");

    QMap<Key, T> ret;
    for (auto it = map.constBegin(); it != map.constEnd(); ++it) {
        QObject *obj = qvariant_cast<QObject *>(it.value());
        if (T casted = qobject_cast<T>(obj))
            ret.insert(Key(it.key()), casted);
    }
    return ret;
}

template<typename Key, typename T>
QVariantMap qMapToVariant(const QMap<Key, T> &map) {
    static_assert(std::is_convertible_v<Key, QString>,
                  "qMapToVariant only supports keys convertible to QString");

    static_assert(std::is_pointer_v<T>,
                  "qMapToVariant assumes T is a pointer to QObject");

    QVariantMap ret;
    for (auto it = map.constBegin(); it != map.constEnd(); ++it) {
        QObject *obj = qobject_cast<QObject *>(it.value());
        if (obj)
            ret.insert(QString(it.key()), QVariant::fromValue(obj));
    }
    return ret;
}
...
public Q_SLOTS: // virtual methods API for QML
    ...
    virtual bool containsUid(const QString &uid) const = 0;
    ...
    // virtual void fromVarArray (const QVariantList &list) = 0; // implement
    virtual QVariantMap toVarMap (void) const = 0;
    // virtual void fromVarMap(const QVariantMap &map) = 0; // implement
    virtual const QMetaObject* itemMetaObject() const = 0;
...
template<class ItemType> class ObjectListModel : public ObjectListModelBase {
public:
        ...
        , m_metaObj(ItemType::staticMetaObject)
    {
        ...
        for (int propertyIdx = 0, role = (baseRole() +1); propertyIdx < len; propertyIdx++, role++) {
            ...
            if (!roleNamesBlacklist.contains(propName)) {
                ...
                static const QByteArray CLASS_NAME = (QByteArrayLiteral("ObjectListModel<") % m_metaObj.className() % '>');
...
public: // QML slots implementation
    ...
    void removeAt(int idx) Q_DECL_FINAL {
        remove(idx);
    }

    bool containsUid(const QString &uid) const Q_DECL_FINAL {
        return (!m_indexByUid.isEmpty() && m_indexByUid.contains(uid));
    }
    ...
    QVariantMap toVarMap(void) const Q_DECL_FINAL {
        QVariantMap ret;
        for (auto it = m_indexByUid.constBegin(); it != m_indexByUid.constEnd(); ++it) {
            ItemType *obj = qobject_cast<ItemType *>(it.value());
            if (obj)
                ret.insert(it.key(), QVariant::fromValue(obj));
        }
        return ret;
    }
    const QMetaObject* itemMetaObject() const override {
        return &ItemType::staticMetaObject;
    }
...
#define QP_MODEL_RO(type, name) \
    protected: Q_PROPERTY (ObjectListModelBase * name READ get_##name CONSTANT) \
    private: ObjectListModel<type> * m_##name; \
    public: ObjectListModel<type> * get_##name (void) const { return m_##name; } \
    private:


#define QP_MODEL_RW(type, name) \
protected: Q_PROPERTY (ObjectListModel<type> * name READ get_##name WRITE set_##name NOTIFY name##Changed () ) \
    private: ObjectListModel<type> * m_##name; \
    public: \
    ObjectListModel<type> * get_##name (void) const { return m_##name; } \
    void set_##name ( ObjectListModelBase<type> * model ) { \
        if ( !m_##name ->isEmpty() ) { \
            m_##name ->clear () ; \
        } \
        m_##name = model ; \
