...
class TmpFile {
public:
    explicit TmpFile(std::string path) :
        m_path(std::move(path))
    {
        std::ofstream stream(m_path, std::ios::binary);
    }

    TmpFile(std::string path, std::size_t size, char pattern) :
        m_path(std::move(path))
    {
        std::ofstream stream(m_path, std::ios::binary);
    ...
    TmpFile(std::string path, const std::vector<std::byte> &data) :
        m_path(std::move(path))
    {
        std::ofstream stream(m_path, std::ios::binary);
    ...
    TmpFile(std::string path, std::string_view data) :
        m_path(std::move(path))
    {
        std::ofstream stream(m_path, std::ios::binary);
        if (!stream)
